...
local_database = r"C:\Users\vinayak\Desktop\Backtesting\Database\database.db"
...
'             OPTIONS BEGINS            '
'---------------------------------------'
def fetch_options_data_on(symbol, date, expiry_type, db_type=DB.QDAP, t=0):
...
def fetch_bhav_data_options_on(symbol, date, expiry_type, db_type=DB.QDAP):
    cursor, conn = make_connection_to_db(db_type)
...
'             FUTURE BEGINS             '
'---------------------------------------'
def fetch_futures_data_on(symbol, date, expiry_type, db_type=DB.QDAP, t=0):
    if db_type == DB.LocalDB and t == 0:
        raise ValueError("Please Enter the timeframe of Data when fetching from local database")
    cursor, conn = make_connection_to_db(db_type)
    if db_type == DB.QDAP:
...
def fetch_bhav_data_futures_on(symbol, date, expiry_type, db_type=DB.QDAP):
    cursor, conn = make_connection_to_db(db_type)
...
'             OPTIONS BEGINS            '
'---------------------------------------'
def get_options_data_on(symbol, date, expiry_type, t = 1):
    print(f"Date: {date}")
    try:
        print(f"Checking if {symbol} {expiry_type}-Options Data exists for this day Locally")
        df = read_data(symbol, date, expiry_type, FNO.OPTIONS)
        if not df.empty:
            print(f"Data Found Locally")
    except:
        print(f"This day's data is not available Locally")
        print(f"Checking if data exists for this day in QDAP")
        df = fetch_options_data_on(symbol, date, expiry_type, DB.QDAP)
        if df.empty:
            print("Uh-oh: Data for this day does not exist in QDAP!")
            print("-----------------------------------------------------------------")
            return
        print(f"Data Found for this day in QDAP")
        df = Data_Processing.drop_duplicates_from_df(df)
        save_data(df, symbol, date, expiry_type, FNO.OPTIONS) 
...
def get_options_data_with_timestamps_between(symbol, start_date, end_date, expiry_type, t=1):
    ...
    for date in expected_date_range:
        df = get_options_data_on(symbol, date.date(), expiry_type, t)
        if df is not None:
...
def get_bhav_data_options_on(symbol, date, expiry_type):
    date = pd.to_datetime(date).date()
    try:
        print(f"Checking if Options Data exists for this day in Local DB")
        df = fetch_bhav_data_options_on(symbol, date, expiry_type, DB.LocalDB)
        if not df.empty:
    ...
    if df.empty:
        print(f"Checking if data exists for this day in QDAP")
        df = fetch_bhav_data_options_on(symbol, date, expiry_type, DB.QDAP)
        if df.empty:
...
def get_bhav_data_options_with_timestamps_between(symbol, start_date, end_date, expiry_type):
    ...
    for date in expected_date_range:
        print(date)
        df = get_bhav_data_options_on(symbol, date.date(), expiry_type)
        if df is not None:
            df['date_timestamp'] = pd.to_datetime(df['date_timestamp'])
            dfs.append(df)
    if len(dfs) != 0:
...
'             FUTURE BEGINS             '
'---------------------------------------'
def get_futures_data_on(symbol, date, expiry_type, t = 1):
    print(f"Date: {date}")
    try:
        print(f"Checking if {symbol} {expiry_type}-Futures Data exists for this day Locally")
        df = read_data(symbol, date, expiry_type, FNO.FUTURES)
        if not df.empty:
            print(f"Data Found Locally")
    except:
        print(f"This day's data is not available locally")
        print(f"Checking if data exists for this day in QDAP")
        df = fetch_futures_data_on(symbol, date, expiry_type, DB.QDAP)
        if df.empty:
            print("Uh-oh: Data for this day does not exist in QDAP")
            print("-----------------------------------------------------------------")
            return  
        print(f"Data for this day found in QDAP")
        df = Data_Processing.drop_duplicates_from_df(df)
        save_data(df, symbol, date, expiry_type, FNO.FUTURES)
        print(f"Data Saved Locally!")
...
def get_futures_data_with_timestamps_between(symbol, start_date, end_date, expiry_type, t=1):
    start_date = pd.to_datetime(start_date)
    end_date = pd.to_datetime(end_date)
    expected_date_range = pd.date_range(start=start_date, end=end_date, freq='B')
    expected_date_range = get_market_valid_days(expected_date_range)
    ...
    for date in expected_date_range:
        df = get_futures_data_on(symbol, date.date(), expiry_type, t)
        if df is not None:
            df['date_timestamp'] = pd.to_datetime(df['date_timestamp'])
            dfs.append(df)
    if len(dfs) != 0:
...
def get_bhav_data_futures_on(symbol, date, expiry_type):
    date = pd.to_datetime(date).date()
    try:
        print(f"Checking if BHAV Futures Data exists for this day in Local DB")
        df = fetch_bhav_data_futures_on(symbol, date, expiry_type, DB.LocalDB)
        if not df.empty:
            print(f"Data Found in Local DB")
            print("-----------------------------------------------------------------")
            df.rename(columns={'data_date': 'date_timestamp'}, inplace=True)
    ...
    if df.empty:
        print(f"Checking if data exists for this day in QDAP")
        df = fetch_bhav_data_futures_on(symbol, date, expiry_type, DB.QDAP)
        if df.empty:
            print("Data for this day does not exist in QDAP!")
            print("-----------------------------------------------------------------")
            return    
...
def get_bhav_data_futures_with_timestamps_between(symbol, start_date, end_date, expiry_type):
    ...
    for date in expected_date_range:
        print(date)
        df = get_bhav_data_futures_on(symbol, date.date(), expiry_type)
        if df is not None:
            df['date_timestamp'] = pd.to_datetime(df['date_timestamp'])
            dfs.append(df)
    if len(dfs) != 0:
...
def get_bhav_data_futures_for_dates(symbol, dates, expiry_type):
...
DATABASE_PATH = r"C:\Users\vinayak\Desktop\Backtesting\Database"


def save_data(df, symbol, date, expiry_type, fno):
    prefix_folder_path = os.path.join(DATABASE_PATH, fno.name)
...
def read_data(symbol, date, expiry_type, fno, raiseError = True):
    prefix_folder_path = os.path.join(DATABASE_PATH, fno.name)
    folder_path = os.path.join(prefix_folder_path, symbol)
    file_name = f'{expiry_type}_{date}'
    file_path = os.path.join(folder_path, f"{file_name}.csv")
...
#         df = fetch_options_data_on(symbol, date, expiry_type, DB.LocalDB, t)
#         if not df.empty:
...
#             df = fetch_options_data_on(symbol, date, expiry_type, DB.LocalDB, 1)
#             if df.empty:
